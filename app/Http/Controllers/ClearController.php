...
class ClearController extends Controller
{
    ...
    public function update(
        ...
        $clearance = Clearance::where('id', $validated['clearance_id'])->first();
        $pending = $clearance->pending();

        if($validated['role']=='heade_master'){
            if($pending != 1){
                return redirect()->back()->withError('Headmaster Clear only if all other clearances are approved !');
            }
        }
